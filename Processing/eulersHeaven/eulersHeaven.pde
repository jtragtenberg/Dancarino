...
void draw() {
  ...
  while (serial.available () >= 12) {
    yaw = radians(readFloat(serial));
    pitch = radians(readFloat(serial));
    roll = radians(readFloat(serial));
  }
  /*
  // Draw board
   pushMatrix();
  ...
  float x = sin(yaw - yawOffset)*cos(pitch);
  float y = -sin(pitch);
  float z = -cos(yaw - yawOffset)*cos(pitch);

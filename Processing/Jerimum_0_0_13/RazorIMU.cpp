...
//casulo
void RazorIMU::getGyro_float()
{
    read_sensors();
    compensate_sensor_errors();
    Serial.write((byte*) gyro,4); //manda componente x do giroscópio
    //return gyro[0];
}

int8_t RazorIMU::getGyro_byte(int eixo)
{
    int8_t gyroByte;
    read_sensors();
    compensate_sensor_errors();
    float giroscopio = gyro[eixo] / 100; //pegando o gyro.x (pode mudar para pegar o eixo y ou z do giroscopio); pode mudar também o fator de escala de 100 para o valor que quiser...
    if (giroscopio > 127) giroscopio = 127;
    else if (giroscopio < -128) giroscopio = -128;
    gyroByte = (int8_t)giroscopio;
    return gyroByte;
}

int8_t RazorIMU::getAccel_byte(int eixo)
{
    int8_t accelByte;
    read_sensors();
    compensate_sensor_errors();
    float acelerometro = accel[eixo] / 2; //pegando o accel.x (pode mudar para pegar o eixo y ou z do acelerometro);
    if (acelerometro > 127) acelerometro = 127;
    else if (acelerometro < -128) acelerometro = -128;
    accelByte = (int8_t)acelerometro;
    return accelByte;
}
